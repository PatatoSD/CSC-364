...
class GameBoard:
    ...
    def occupied_spots(self):
        return {p.position.as_tuple() for p in self.all_players}
    ...
    def run_turn(self):
        ...
        turn_start_pos = {p: p.position for p in self.all_players}

        occupied = self.occupied_spots()
        carrier = self.possession_player

        resolution_order = self.all_players[:]
        ...
        for player in resolution_order:
            ...
            if proposed == carrier.position and player is not carrier and is_enemy_of_carrier:
                ...
                occupied.discard(old_carrier.position.as_tuple())
                old_carrier.position = turn_start_pos[old_carrier]
                occupied.add(old_carrier.position.as_tuple())
                continue
    ...
    def reset_after_goal(self, conceding_team):
        for p in self.all_players:
            p.reset()
        self.possession_player = conceding_team.players[len(conceding_team.players) - 1]